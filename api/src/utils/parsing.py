...
from src.schemas.config_gambit import GAMBIT_CONFIG
from src.schemas.constants import FIELD_STRING_REGEX_GROUPS
from src.schemas.template_gambit import GAMBIT_TEMPLATE
...
def open_config_with_defaults():
    user_tuning_config = OVERRIDE_MERGER.merge(
        deepcopy(CONFIG_DEFAULTS), deepcopy(GAMBIT_CONFIG)
    )
    validate_config_json(user_tuning_config)
    # https://github.com/drgrib/dotmap/issues/74
    return DotMap(user_tuning_config, _dynamic=False)


def open_template_with_defaults():
    user_template = OVERRIDE_MERGER.merge(deepcopy(TEMPLATE_DEFAULTS), deepcopy(GAMBIT_TEMPLATE))
    validate_template_json(user_template)
